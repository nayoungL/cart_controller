...
bool process(cv::Mat test_img, std::vector<geometry_msgs::Point32>& waypoint, int& return_id){
...
///color
    ...
    cv::Scalar lower_white=cv::Scalar(0,0,170);
    cv::Scalar upper_white=cv::Scalar(120,100,255);
...
////////////////marker//////////////////
    ...
    std::cout << std::endl;
    return_id=-1;
    if(line_center_pt.size()>2){
        if(std::abs(line_center_pt[0].slope) >= 4){
            for(int i=0; i<line_center_pt.size(); i++){
                int first_x=line_center_pt.at(0).x;
                int last_x=line_center_pt.at(line_center_pt.size()-1).x;
                int mid_x=(first_x+last_x)/2;
                std::cout << mid_x-100 << " "<< line_center_pt[i].x <<" "<< mid_x+100 << std::endl;
                if(line_center_pt[i].x>mid_x-100 && line_center_pt[i].x<mid_x+100){
                    line_center_pt.erase(line_center_pt.begin()+i);
                    if(is_marker==false){
    ...
    std::cout << "marker" << std::endl;
    std::cout << marker_f_cnt << std::endl;
    std::cout << is_marker << std::endl;
    if(marker_f_cnt>500){
        is_marker=false;
        marker_f_cnt=0;
    }
...
int main(int argc, char** argv){
    ...
    ros::Publisher PubMarker=nh.advertise<std_msgs::Int8> ("/lane_marker",1);
    ...
    while(ros::ok()){
        ...
        if(!rawImage.empty()){
            ...
            if(process(rawImage, waypoint, publish_id)){
